...
type Method = "euclid" | "prime" | "divisors"
...
type Problem = { nums: number[]; gcd: number }
...
function formatFactorizationHtml(factors: number[]) {
...
const gcdResult = computed(() => {
  if (calculator.numbersCount === 3) {
    return gcdMany([calculator.a, calculator.b, calculator.c])
  }
  return gcd(calculator.a, calculator.b)
})
const euclidSteps = computed(() => gcdSteps(calculator.a, calculator.b))
const primeInfo = computed(() => {
  ...
  return {
    ...
    factors,
    common,
    gcd: common.reduce((acc, val) => acc * val, 1) || 1,
    factorsHtml: factors.map((list) => formatFactorizationHtml(list)),
  }
})
...
const divisorInfo = computed(() => {
  const values = calculator.numbersCount === 3
    ? [calculator.a, calculator.b, calculator.c]
    : [calculator.a, calculator.b]
  const divisorLists = values.map((value) => divisors(value))
  const common = divisorLists.reduce((acc, list) => acc.filter((val) => list.includes(val)))
  return {
    values,
    divisorLists,
    common,
    gcd: common[common.length - 1] || 1,
  }
})
...
function generateProblems() {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    list.push({ nums, gcd: gcdMany(nums) })
  }
  problems.value = list
}
...
async function downloadPdf() {
  ...
  try {
    ...
    const payload = {
      ...
      title: "GCD / HCF Practice",
    }
    const blob = await api<Blob>("math/gcd/printouts/pdf", "POST", {
      auth: true,
      body: payload,
      responseType: "blob",
    })
    ...
    link.href = url
    link.download = `gcd_hcf_${practice.outputType}_printouts_${stamp}.pdf`
...
async function downloadCalculatorPdf() {
  ...
  try {
    ...
    const blob = await api<Blob>("math/gcd/calculator/pdf", "POST", {
      auth: true,
      body: payload,
      responseType: "blob",
    })
    ...
    const link = document.createElement("a")
    link.href = url
    link.download = `gcd_hcf_calculator_${stamp}.pdf`
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)
...
async function downloadClassPdf() {
  ...
  try {
    ...
    const blob = await api<Blob>("math/gcd/printouts/class/pdf", "POST", {
      auth: true,
      body: payload,
      responseType: "blob",
    })
    ...
    const link = document.createElement("a")
    link.href = url
    link.download = `gcd_hcf_${slug}_${stamp}.pdf`
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)
...
async function pushClassToQuiz() {
  ...
  try {
    ...
    await api("math/gcd/quiz", "POST", { auth: true, body: payload })
  ...
  <section class="gcd">
    <div class="gcd-header">
      <div>
        ...
        <h1>GCD / HCF Explorer</h1>
        <p class="subtle">Compute greatest common divisors and generate practice worksheets.</p>
      </div>
      ...
      <section class="card">
        ...
        <label>
          ...
          <select v-model="calculator.method">
            ...
            <option value="divisors">List common divisors</option>
          </select>
        </label>
        <label class="toggle">
        ...
        <div class="result">
          <span class="stat-label">GCD / HCF</span>
          <span class="stat-value">{{ gcdResult }}</span>
        </div>
        ...
        <div v-if="calculator.showSteps && calculator.method === 'euclid'" class="steps">
          <h3>Euclidean steps</h3>
          <ol>
            <li v-for="(step, idx) in euclidSteps" :key="idx">
              {{ step.a }} = {{ step.b }} × {{ Math.floor(step.a / step.b) }} + {{ step.r }}
            </li>
          </ol>
        </div>
        ...
        <div v-if="calculator.showSteps && calculator.method === 'prime'" class="steps">
          ...
          <p>Common factors: {{ primeInfo.common.join(" × ") || "None" }}</p>
          <p><strong>GCD = {{ primeInfo.gcd }}</strong></p>
        ...
        <div v-if="calculator.showSteps && calculator.method === 'divisors'" class="steps">
      ...
      <section class="card">
        ...
        <label>
          ...
          <select v-model="practice.method">
            <option value="prime">Prime factorization</option>
            <option value="divisors">List divisors</option>
            <option value="euclid">Euclidean algorithm</option>
          </select>
        </label>
        <label>
        ...
        <div class="practice-list">
          <div v-for="(item, idx) in problems" :key="`${item.nums.join('-')}-${idx}`" class="practice-row">
            <span>{{ idx + 1 }}. HCF({{ item.nums.join(", ") }})</span>
            <span v-if="practice.showAnswers" class="answer">{{ item.gcd }}</span>
